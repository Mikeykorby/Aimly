...
namespace Aimmy2.Controls
{
    public partial class ModelMenuControl : UserControl
    {
        ...
        private void RefreshLocalButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                _mainWindow?.fileManager.LoadModelsIntoListBox(null, null);
                _mainWindow?.fileManager.LoadConfigsIntoListBox(null, null);
                new NoticeBar("Local library refreshed.", 2500).Show();
            }
            catch (Exception ex)
            {
                new NoticeBar($"Failed to refresh local library: {ex.Message}", 5000).Show();
            }
        }

        private void OpenModelsButton_Click(object sender, RoutedEventArgs e) => OpenLocalFolder("models");

        private void OpenConfigsButton_Click(object sender, RoutedEventArgs e) => OpenLocalFolder("configs");

        private void CopySetupButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var summary = _mainWindow?.GetActiveSetupSummary() ?? "No active setup available";
                Clipboard.SetText(summary);
                new NoticeBar("Active setup copied to clipboard.", 2500).Show();
            }
            catch (Exception ex)
            {
                new NoticeBar($"Failed to copy active setup: {ex.Message}", 5000).Show();
            }
        }

        private static void OpenLocalFolder(string folder)
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "bin", folder);
