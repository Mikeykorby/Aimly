    ...
    public partial class SettingsMenuControl : UserControl
    {
        ...
        private void LoadModelSettings()
        {
            ...
            builder
                ...
                .AddSeparator();
            ApplyLastButtonStyling(ModelSettingsPanel);
        }
        ...
        private void LoadSettingsConfig()
        {
            ...
            builder
                ...
                .AddButton("Reload Current Config", b => b.Reader.Click += (s, e) => _mainWindow?.ReloadCurrentConfig(),
                    tooltip: "Reapply the currently loaded config without reopening the file list.")
                .AddButton("Copy Active Setup", b => b.Reader.Click += (s, e) =>
                {
                    var summary = _mainWindow?.GetActiveSetupSummary() ?? string.Empty;
                    System.Windows.Clipboard.SetText(summary);
                    LogManager.Log(LogLevel.Info, "Active setup summary copied to clipboard.", true);
                }, tooltip: "Copy the current model, config, and display to your clipboard.")
                .AddButton("Open Models Folder", b => b.Reader.Click += (s, e) => OpenFolder("bin\\models"),
                    tooltip: "Open the folder that contains your AI models.")
                .AddButton("Open Configs Folder", b => b.Reader.Click += (s, e) => OpenFolder("bin\\configs"),
                    tooltip: "Open the folder that contains saved configs.")
                .AddSeparator();
            ApplyLastButtonStyling(SettingsConfigPanel);
        }
        ...
        private void LoadDisplaySelectMenu()
        {
            ...
            ApplyLastButtonStyling(DisplaySelectMenuPanel);
        }
        ...
        private void LoadThemeMenu()
        {
            ...
            builder
                .AddTitle("Theme Settings", true, t =>
                {
                    ...
                    // Keep the title bar compact for this section so it doesn't stretch full width
                    t.Width = 284.0;
                    t.Minimize.Click += (s, e) =>
        ...
        private void ApplyLastButtonStyling(StackPanel panel)
        {
            if (panel == null || panel.Children.Count == 1) return;

            for (int i = panel.Children.Count - 1; i >= 0; i--)
            {
                if (panel.Children[i] is APButton button)
                {
                    button.ApplyLastButtonStyle();
                    return;
                }
            }
        }
        ...
        public void UpdateImageSizeDropdown(string newSize)
        {
            if (_mainWindow?.uiManager.D_ImageSize != null)
            {
                ...
                for (int i = 5; i < dropdown.DropdownBox.Items.Count; i++)
                {
